          ...
          <Avatar className="mr-2 h-5 w-5">
            ...
            <AvatarFallback>
              { isLoading ? (
                ...
                <div className="h-full w-full" style={{background: `linear-gradient(-45deg, ${stringToHslColor(String(selectedBoard?.name))} 25%, ${stringToHslColor(String(selectedBoard?.name) + "yeet")} 75%)`}}/>
              )}
            </AvatarFallback>
          </Avatar>
                  ...
                  <Link href={`/board/${board.id}`} key={index}>
                    ...
                    >
                      <Avatar className="mr-2 h-5 w-5">
                        ...
                        <AvatarFallback>
                          <div
                            style={{background: `linear-gradient(-45deg, ${stringToHslColor(String(board.name))} 25%, ${stringToHslColor(String(board.name) + "yeet")} 75%)`}}
            ...
            <CommandGroup>
              <CommandItem
                onSelect={() => {
                  setOpen(false);
                  createBoard.mutate({
                    name: `${sessionData?.user?.name ? `${sessionData?.user?.name}'s` : "New"} Board`,
                  });
                }}
